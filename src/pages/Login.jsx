        ...
        <div className="hidden md:flex flex-col flex-1 max-w-xl space-y-12">
          ...
          <div className="space-y-6">
            <h1 className="text-5xl font-extrabold tracking-tight text-on-surface leading-[1.1]">
              {role === 'employer' ? (
                <>Hire with <span className="text-primary signature-gradient bg-clip-text text-transparent">surgical</span> precision.</>
              ) : (
                <>Find your next role with <span className="text-primary signature-gradient bg-clip-text text-transparent">AI</span> precision.</>
              )}
            </h1>
          ...
          <div className="space-y-8 pt-4">
            <div className="flex items-start gap-4">
              <div className="mt-1 w-8 h-8 rounded-full bg-secondary-container flex items-center justify-center shrink-0">
                <span className="material-symbols-outlined text-on-secondary-container text-sm">verified_user</span>
              </div>
              <div>
                <h3 className="font-bold text-on-surface">Precision Curation</h3>
                <p className="text-on-surface-variant text-sm">Every candidate profile is treated as an editorial entry, scored for cultural and technical fit.</p>
              </div>
            </div>
            {/* ... other items ... */}
          </div>
          ...
          <div className="bg-surface-container-lowest p-8 md:p-10 rounded-3xl shadow-2xl shadow-on-surface/5 flex flex-col border border-outline-variant/10">
            ...
            <div className="space-y-4">
              ...
              <div className="relative flex py-4 items-center">
                ...
                <span className="flex-shrink mx-4 text-outline text-[10px] font-bold tracking-[0.2em] uppercase">Or use email</span>
                <div className="flex-grow border-t border-outline-variant/30"></div>
              </div>
              <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); window.location.href = role === 'employer' ? '/onboarding' : '/seeker/onboarding'; }}>
                <div className="space-y-2">
                  <label className="text-xs font-black tracking-widest text-on-surface-variant uppercase ml-1" htmlFor="email">Email Address</label>
                  <input className="w-full px-5 py-4 bg-surface-container-low border-none rounded-2xl focus:ring-4 focus:ring-primary/10 transition-all font-medium" id="email" placeholder={role === 'employer' ? "curator@kuantum.ai" : "professional@kuantum.ai"} type="email" />
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between items-center px-1">
                    <label className="text-xs font-black tracking-widest text-on-surface-variant uppercase" htmlFor="password">Password</label>
                    <a className="text-[10px] font-black text-primary hover:underline tracking-widest uppercase" href="#">Forgot?</a>
                  </div>
                  <input className="w-full px-5 py-4 bg-surface-container-low border-none rounded-2xl focus:ring-4 focus:ring-primary/10 transition-all font-medium" id="password" placeholder="••••••••" type="password" />
                </div>
                <Link to={role === 'employer' ? "/onboarding" : "/seeker/onboarding"} className="w-full inline-block text-center py-5 signature-gradient text-white font-black rounded-2xl shadow-2xl shadow-primary/30 hover:scale-[1.02] active:scale-[0.98] transition-all mt-4 text-lg">
                  {role === 'employer' ? 'Log In' : 'Enter Portal'}
                </Link>
              </form>
            </div>
            <div className="mt-8 text-center">
              <p className="text-sm text-on-surface-variant font-medium">
                ...
                <a className="text-primary font-black hover:underline ml-1" href="#">Start free trial</a>
              </p>
            </div>
          </div>
          <div className="mt-8 flex justify-between px-2">
            <div className="flex gap-6">
              <a className="text-[10px] font-black text-outline hover:text-on-surface transition-colors tracking-widest uppercase" href="#">Privacy</a>
              <a className="text-[10px] font-black text-outline hover:text-on-surface transition-colors tracking-widest uppercase" href="#">Terms</a>
            </div>
            <p className="text-[10px] font-black text-outline tracking-widest uppercase">© 2024 KUANTUM AI</p>
          </div>
        </div>
      </div>
    </div>
